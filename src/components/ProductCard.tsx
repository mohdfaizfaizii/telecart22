  ...
  return (
    <div className="relative w-full ">
      ...
      <div 
        ...
        className={`group relative overflow-hidden rounded-2xl border border-transparent bg-white shadow-[0_10px_30px_rgba(15,23,42,0.10)] transition-all duration-500 ease-out ${showWhatsNext ? 'cursor-default' : 'hover:-translate-y-2 hover:scale-[1.035] hover:border-blue-500 hover:shadow-[0_24px_60px_rgba(15,23,42,0.18)] cursor-pointer'}`}
      >
        <div className={showWhatsNext ? 'pointer-events-none' : ''}>
          {/* Header */}
          <div className="flex items-center gap-3 sm:gap-4 p-4 sm:p-6 pb-3 sm:pb-4">
            {logoUrl ? (
            <div className="flex h-16 w-16 sm:h-20 sm:w-20 flex-shrink-0 items-center justify-center overflow-hidden transition-transform duration-500 ease-out group-hover:scale-110">
              <img src={logoUrl} alt={companyName} className="h-full w-full object-contain" />
            </div>
          ) : (
            <div className="h-16 w-16 sm:h-20 sm:w-20 flex-shrink-0 rounded-xl bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center transition-transform duration-500 ease-out group-hover:scale-110">
              <span className="text-xl sm:text-2xl font-bold text-primary-foreground">{companyName.charAt(0)}</span>
            </div>
          )}
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              <h3 className="text-xl sm:text-2xl font-bold text-foreground truncate transition-colors duration-300 group-hover:text-[#4027bf]">{companyName}</h3>
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault();
            ...
            {subtitle && <p className="text-sm text-muted-foreground mt-2 px-2 py-1 bg-gray-100 rounded-md inline-block">{subtitle}</p>}
          </div>
        </div>

        {/* Description */}
        <div className="px-4 sm:px-6">
          <p className="text-base sm:text-lg text-foreground leading-snug line-clamp-2 transition-colors duration-300 group-hover:text-foreground/90">{description}</p>
        </div>


        {/* Pricing Row */}
        {showPricingBlock && (
          <div className="mx-4 sm:mx-6 mt-4 rounded-xl border border-[#d9dee7] bg-[#f3f4f6] px-4 py-3">
            {showPricingBlock && (
              <div>
                {priceOnRequest ? (
                  <p className="text-lg sm:text-xl font-semibold text-primary">STARTING AT Price on Request</p>
                ) : (
                  <p className="text-xl sm:text-2xl font-semibold text-[#4027bf] whitespace-nowrap">
                    <span className="mr-1 text-base sm:text-lg font-medium uppercase text-muted-foreground">STARTING AT</span>
